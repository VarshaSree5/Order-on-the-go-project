...
const Cart = () => {
  ...
  const mockItems = useMemo(() => [
    {
      ...
      foodItemImg:
        'https://static01.nyt.com/images/2024/10/29/multimedia/Butter-Chickenrex-tbvz/Butter-Chickenrex-tbvz-mediumSquareAt3X.jpg',
      price: 320,
      discount: 10,
      quantity: 2,
    },
  ...
  const fetchCart = useCallback(async () => {
    try {
      ...
      setCart(filtered.length ? filtered : mockItems);
    } catch (error) {
    ...
    <div className="cartPage">
      ...
      <div className="cartPriceBody">
        ...
        <span>
          ...
          <span style={{ color: 'red' }}>+ ₹{deliveryCharges}</span>
        </span>
        <hr />
        <h5>
                ...
                {['Name', 'Mobile', 'Email', 'Address', 'Pincode'].map((label, idx) => (
                  <div className="form-floating mb-3" key={idx}>
                    <input
                      ...
                      id={`checkout-${label.toLowerCase()}`}
                      placeholder={label}
                    />
                    <label htmlFor={`checkout-${label.toLowerCase()}`}>{label}</label>
                  </div>
                ))}
              </div>
